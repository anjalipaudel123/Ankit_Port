...
import type { PhotoCardProps } from '@/types/gallery';
...
export function PhotoCard({ photo, index, onClick, isVisible }: PhotoCardProps) {
  ...
  return (
    <div
      ref={cardRef}
      className={`perspective-container transition-all duration-700 ${
    ...
    >
      <div
        className={`card-3d relative overflow-hidden rounded-lg cursor-pointer bg-[#141414] ${aspectRatioClass}`}
        style={{
          transformStyle: 'preserve-3d',
          transform: `
      ...
      >
        ...
        {/* Gradient Overlay */}
        <div 
          className="absolute inset-0 image-overlay transition-opacity duration-300"
        ...
        {/* Content */}
        <div 
          className="absolute bottom-0 left-0 right-0 p-5 transition-all duration-350"
          style={{ 
            opacity: isHovered ? 1 : 0, 
        ...
        >
          ...
          <button
            className="inline-flex items-center gap-2 px-4 py-2 border border-white/30 rounded-full text-sm text-white hover:bg-[#ff0000] hover:border-[#ff0000] transition-all duration-300 hover:scale-105 active:scale-95"
          >
