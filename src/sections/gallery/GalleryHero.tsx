...
export function GalleryHero({ activeCategory, onCategoryChange, photoCount }: GalleryHeroProps) {
    return (
        <section className="relative flex flex-col items-center sm:justify-center sm:min-h-[55vh] overflow-hidden">
            {/* ── F1-style top ticker bar ── */}
            <div className="absolute top-0 left-0 right-0 z-20 bg-[#e10600] h-7 sm:h-8 flex items-center overflow-hidden">
                <div className="gallery-ticker flex items-center gap-0 whitespace-nowrap">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <span key={i} className="flex items-center gap-4 sm:gap-6 px-4 sm:px-6">
                            <span className="text-white text-[9px] sm:text-[10px] font-black tracking-[0.2em] sm:tracking-[0.25em] uppercase">Gallery</span>
                            <span className="text-white/40 text-[9px] sm:text-[10px]">·</span>
                            <span className="text-white/80 text-[9px] sm:text-[10px] font-mono tracking-widest">{String(photoCount).padStart(2, '0')} Frames</span>
                            <span className="text-white/40 text-[9px] sm:text-[10px]">·</span>
                            <span className="text-white/80 text-[9px] sm:text-[10px] font-bold tracking-[0.2em] uppercase">Season 2026</span>
                            <span className="text-white/40 text-[9px] sm:text-[10px]">·</span>
                        </span>
                    ))}
                </div>
            ...
            {/* F1-style scan-line grid — desktop only */}
            <div className="absolute inset-0 pointer-events-none gallery-scan-grid hidden sm:block" />
            ...
            {/* Content */}
            <div className="relative z-10 text-center max-w-4xl mx-auto px-4 pt-10 sm:pt-20 pb-4 sm:pb-12">

                {/* Main heading — single line on mobile */}
                <div className="overflow-hidden mb-3 sm:mb-8">
                    <h1
                        className="text-3xl sm:text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight gallery-flip-in"
                ...
                {/* F1 telemetry row — hidden on mobile */}
                <div
                    className="hidden sm:flex flex-wrap items-center justify-center gap-px mb-10 gallery-fade-in-up"
            ...
            {/* F1-style bottom event tracker bar — desktop only */}
            <div className="hidden sm:flex absolute bottom-0 left-0 right-0 z-10 h-10 border-t border-gray-200 dark:border-white/10 bg-gray-50/80 dark:bg-[#0a0a0a]/80 backdrop-blur-sm items-center px-6 gap-4">
